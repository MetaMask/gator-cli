...
export function getStorageClient(config: PermissionsConfig) {
  ...
  return {
    ...
    async storeDelegation(delegation: Delegation): Promise<Hex> {
      ...
      const response = await fetch(`${opts.apiUrl}/delegation/store?metadataCategory=*`, {
        method: "POST",
     ...
     */
    async getDelegationChain(leafDelegationOrHash: Delegation | Hex): Promise<Delegation[]> {
      const leafHash: Hex =
